...
* **Nomenclatura para Vue:** Para la codificación de la aplicación web en Vue, se toman como base las recomendaciones oficiales de la guía de estilo de Vue, especialmente en lo referido a la nomenclatura de componentes, propiedades y eventos. Vue recomienda usar **PascalCase** para los nombres de componentes en Single-File Components y en JavaScript/JSX, usar **camelCase** para la declaración de props, y emplear **kebab-case** al invocarlas en plantillas in-DOM o en los listeners de eventos; además, sugiere preferir nombres completos y descriptivos para mejorar la legibilidad y el mantenimiento del código. 
  * Los componentes se nombran en **PascalCase**.  
  * Las props se declaran en **camelCase** y se usan en **kebab-case** cuando corresponde.  
  * Los eventos personalizados se referencian preferentemente en **kebab-case**.  
  * Se priorizan nombres completos y descriptivos para evitar abreviaciones poco claras.  

* **Nomenclatura para Flutter:** Para la aplicación móvil desarrollada con Flutter, se adoptan las convenciones de estilo de Dart, ya que Flutter se construye sobre este lenguaje. La guía oficial indica que los tipos deben usar **UpperCamelCase**, los identificadores como funciones, variables y propiedades deben usar **lowerCamelCase**, y los archivos fuente, directorios y paquetes deben nombrarse en **lowercase_with_underscores**. También se recomienda mantener una nomenclatura estandarizada para clases, archivos y directorios dentro de proyectos Flutter.
  * Las clases, widgets, enums, typedefs y otros tipos se nombran en **UpperCamelCase**.
  * Las funciones, propiedades, variables locales y parámetros usan **lowerCamelCase**.
  * Los paquetes, directorios y archivos fuente se nombran en **lowercase_with_underscores**.
  * En Flutter se mantiene una convención uniforme para facilitar la lectura, la escalabilidad y el mantenimiento del código.
