...
* **Nomenclatura para Vue:** Para la codificación de la aplicación web en Vue, se toman como base las buenas prácticas recomendadas para el desarrollo de componentes reutilizables, organización modular y consistencia en la nomenclatura. A continuación, se detallan las convenciones que se aplican en el proyecto:
  * Los componentes deben nombrarse en **PascalCase**. Por ejemplo: `ProductCard.vue`, `InventoryList.vue`.
  * Los nombres de los archivos de componentes deben coincidir con el nombre del componente y mantenerse en minúsculas o en PascalCase según la convención del equipo, evitando nombres ambiguos.
  * Las variables, funciones y métodos deben usar **camelCase**. Por ejemplo: `productList`, `loadInventory`, `handleSubmit`.
  * Las propiedades (**props**) deben declararse en camelCase dentro del componente y utilizarse en **kebab-case** cuando se invocan en el template.
  * Los eventos personalizados deben tener nombres descriptivos y consistentes, preferentemente en kebab-case. Por ejemplo: `update-stock`, `select-product`.
  * Los composables deben comenzar con el prefijo **use**. Por ejemplo: `useInventory`, `useAuth`, `useProducts`.
  * Cuando se utilice **Pinia**, los stores deben nombrarse según el contexto funcional. Por ejemplo: `useInventoryStore`, `useAuthStore`.
  * Las rutas deben organizarse por módulos o vistas funcionales. Por ejemplo: `/inventory`, `/reports`, `/settings`.
  * En los templates, se debe procurar mantener una estructura limpia, legible y modular, evitando la repetición innecesaria de código.
  * Los estilos deben utilizar clases descriptivas y reutilizables, evitando nombres genéricos o ambiguos. Además, se debe priorizar la separación entre estructura, comportamiento y presentación.
